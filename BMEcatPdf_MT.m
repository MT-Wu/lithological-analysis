function [pk]=BMEcatPdf_MT(ck,cs,ps,dmodel,Pmodel,Betarange,nsmax,dmax,options);
...
% 最大選取數量           for the estimation at the locations specified in ck.
% dmax      scalar     maximum distance between an estimation location and
% 最大選取距離           locations where categories are known. All locations
...
for i=1:nk,
  ...
  if sumnslocal>0,
    ...
    Pkh=maxentropytable_MT([ck0;cslocal],dmodel,Pmodel,Betarange,options(2)); %%改過
