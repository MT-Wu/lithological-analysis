...
ncat=size(Pmodel,1); %類別數（岩性是三類）
ndim=size(c,1); %欲推估點的ck0加上周圍取到的cslocal數量(nmax)->ndim=位置點數量nmax+1

%%%%%% Build the bivariate probability tables

Pbiv=cell(ndim,ndim);
Pbeta=cell(ndim,ndim);
for i=1:ndim, %算取的坐標兩兩之間的距離
  for j=i+1:ndim,
    d=coord2dist(c(i,:),c(j,:));
    Pbiv{i,j}=probamodel2bitable(d,dmodel,Pmodel); %線性內插出兩兩之間的機率
    Pbeta{i,j}=probamodel2bitable(d,dmodel,Betarange); %線性內插出兩兩之間的機率誤差範圍
  end;       %建立取的位置點之間的機率表
end;

%%%%%% Estimate the maximum entropy table

[Pfit,niter]=iterativerescaling_MT(Pbiv,ndim,ncat,Pbeta,tol); %Pbeta是另外加上去的
